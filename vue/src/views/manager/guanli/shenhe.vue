        ...
        <el-table :data="data.tableData" height="440" style="width: 100%">
          ...
          <el-table-column fixed="right" align="center" label="操作" width="160" >
            <template #default="scope">
              ...
              <el-button type="info" size="small" @click="handleadd(scope.row.id)">复审</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <el-dialog width="50%" v-model="data.formVisible" title="审核">
      <el-form :model="data.form" ref="formRef" :rules="rules">
        <el-form-item style="margin-right: 40px" label="审核回复" :label-width="formLabelWidth" >
          <el-input v-model="data.form.fuhui" autocomplete="off" />
        </el-form-item>
        <el-form-item label="是否通过" :label-width="formLabelWidth" prop="chushen">
          <el-select v-model="data.form.fushen" placeholder="请选择">
            <el-option label="复审过" value="复审过" />
...
// console.log(data.form.time)

// 此处初审过后，公示三天，复审才自动显示
const load = () => {
  request.get('/time/select', {
    params: {
      dangqian:data.form.time,
      zhuangtai:"初审过",
      fushen:"未审核",
    }
  }).then(res => {
...
const save=()=>{
  ...
  data.form.futime=nowDate(time)

  formRef.value.validate((valid)=>{
    if(valid){
      request.put('/Sh/fushen',data.form).then(res=>{
        if(res.code==='200'){
